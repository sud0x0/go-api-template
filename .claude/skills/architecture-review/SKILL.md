...
- Pagination sizes come from `shared.DefaultPageSize`/`MaxPageSize`/`MaxPageOffset`; batch cap from `shared.MaxBatchSize`; length from `shared.LogMaxChars`. `grep -rn '\b100\b\|\b10000\b' internal/<feature>/` and confirm each is a named constant, not a re-hardcoded literal.
