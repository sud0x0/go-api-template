...
| 5 · Authorise in the repository | **8.2.2, 8.3.1, 8.4.1**. Function/attribute-level via OPA is **8.2.1** (#17, #21) | Every query `WHERE … AND user_id = $N` is the ROW-OWNERSHIP layer, scoped to the caller OR an OPA-authorised target. No row filtering in the handler. `grep -rn 'user_id = \$' <scope>`: one per query, and **no query lacking a `user_id` predicate** was added (no unscoped whole-table read). OPA does FUNCTION/ATTRIBUTE-level authz only. Confirm no change pushes row filtering into OPA (the two layers stay separate) |
...
3. **Template-specific checks** (each annotated with the ASVS requirement it operationalises):
   ...
   - **Cross-user / ABAC** (8.2.1, 8.2.2, 8.3.1; #21): if the change adds or touches cross-user access, verify EVERY authorisation attribute (subject, roles, any boundary attribute such as team) comes from the validated token or a trusted store keyed by the subject, **never from request input**. The target user MAY be request-supplied (`?user=`), but it must be `uuid.Parse`-validated (→ 400 `invalid_target`, never a 500) BEFORE the OPA input and any query, and its authorisation is decided by OPA PER OBJECT (fail-closed: only an explicit `true` allows; the target is set on the context only after that allow). The repository must still scope to the **target's** `user_id` via a `*ForTarget` path — confirm no unscoped whole-table query was introduced (`grep -rn 'FROM logs' <scope>` → every read/write carries a `user_id` predicate). Create stays caller-owned. Verdicts cite `authz_middleware.go`, `policy.rego`, and the cross-user tests.
