...
| ID | Requirement (paraphrased) | L | Status | Control / file | Test evidence |
|----|---------------------------|---|--------|----------------|----------------|
| 8.1.1 | Docs define function-level and data-specific access rules based on permissions/attributes | 1 | met | ATTRIBUTE-BASED (ABAC): the OPA policy decides on caller attributes (`subject`, `roles`) AND an object attribute (`target_user`), so both function/attribute-level rules (role-only `admin` + scoped `team-lead` boundary in `deploy/opa/policy.rego`, `decisions.md` #21) and data-specific row-ownership rules (`security.md` rule 5 / `decisions.md` #10) are documented | `policy_test.rego` (self/admin/team-lead/deny cases) |
| 8.1.2 | Docs define field-level (read/write) access restrictions | 2 | n/a | No field-level authorization; OPA authorises at function/attribute level (caller + target owner), not per field (`decisions.md` #17, #21) | — |
| 8.2.1 | Function-level access restricted to consumers with explicit permissions | 1 | met | OPA sidecar authorises `{subject, roles, action, resource, method, target_user}` fail-closed before the handler runs; cross-user access is allowed only on an explicit policy `true`. `resource` is the chi LEAF pattern (e.g. `/api/v1/logs/{id}`), guaranteed by post-routing (inline-group) wiring in `middleware.AuthorizedRoutes` — a pre-routing `r.Use` would send the mount `/api/v1/*` and deny everything. Roles are DENY BY DEFAULT (`mapClaimsToRoles` trusts no IdP name). (`internal/middleware/authz_middleware.go`, `internal/middleware/routing.go`, `deploy/opa/policy.rego`, `decisions.md` #17/#21) | `TestOPAAuth_ResourceIsLeafPattern_RealRouter`, `TestOPAAuth_FailsClosed`, `TestOPAAuth_AllowsOnlyOnExplicitTrue`, `TestOPAAuth_CrossUserDeniedFailsClosed`, `TestMapClaimsToRoles`, `policy_test.rego` |
| 8.2.2 | Data-specific access restricted per-consumer (mitigate IDOR/BOLA) | 1 | met | Every query is `WHERE … AND user_id = $N` (self OR OPA-authorised target); cross-user uses a target-scoped path with NO unscoped whole-table query — `internal/userlog/userlog_repository.go`; identity boundary at `internal/shared/identity.go` (UUID-enforced, 401 on bad subject; `target_user` UUID-validated → 400) | `TestLogRepository_CrossUserIsolation_Integration`, `TestLogHandler_CrossUser`, `TestLogHandler_CrossUser_Integration`, `TestUserIDFromContext_RejectsNonUUID`, `TestTargetUserFromContext` |
...
| 8.3.1 | Enforce authorization at a trusted service layer, not client-manipulable controls | 1 | met | Row-ownership enforced server-side in the repository SQL, not the handler; the cross-user target is authorised by OPA and re-scoped to the target's `user_id` in SQL, never trusted from request input — `internal/userlog/userlog_repository.go`; `security.md` rule 5 | `TestLogRepository_CrossUserIsolation_Integration`, `TestLogHandler_CrossUser_Integration` |
| 8.4.1 | Multi-tenant cross-tenant controls so operations never affect other tenants | 2 | met | Per-user isolation is the tenancy boundary: all reads/writes scoped by `user_id`; cross-user access is a SEPARATE OPA-gated path still scoped to the target's `user_id` (never unscoped), and forged cursors stay within one owner's rows (`decisions.md` #3, #21) | `TestLogRepository_CrossUserIsolation_Integration`, `TestLogHandler_CrossUser` |
...
| 9.2.2 | Validate token TYPE / purpose; only accept access tokens for authorization | 2 | met | PRIMARY guard is the audience check with `OIDC_AUDIENCE` = this API's own identifier (NOT a browser client_id), so an ID token (`aud`=client_id) is rejected; layered positive detection rejects `token_use=="id"` (Cognito) AND OIDC-Core ID-token-only claims `nonce`/`at_hash`/`c_hash` (present in no access token) (`internal/middleware/auth_middleware.go`, `internal/config/config.go`) | `TestOIDCAuth_RejectsIDToken` (token_use + nonce/at_hash/c_hash, all with aud==audience) |
